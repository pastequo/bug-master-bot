...
CONFIGURATION_FILE_NAME = os.getenv(
    "CONFIGURATION_FILE_NAME", default="bug_master_configuration.yaml"
)
...
if SIGNING_SECRET is None:
    raise EnvironmentError(
        "Missing signing secret (SIGNING_SECRET) environment variable"
    )
if BOT_USER_TOKEN is None:
    raise EnvironmentError(
        "Missing bot user token (BOT_USER_TOKEN) environment variable"
    )
